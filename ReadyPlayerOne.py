...
conn = sqlite3.connect('sqlite:///PyHearts.db')
...
class Player():
	...
	def score_pile(self):
		...
		#I'm not entirely sure how I want to analyze shooting to moon so I'll just leave it as zero impact for now
		# now we can add the score increase to our score
		self.score += score_increase
		self.qp	= qp
		self.hp = hp
		cur.execute("INSERT INTO PlayerGBG VALUES (?,?,?)", (self.name, hp, qp))
		self.qp = 0
		self.hp = 0
